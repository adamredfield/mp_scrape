...
def set_route_finder_filters(page, grade, state):
    ...
    try:
        ...
        page.select_option(
            'select[id="diffMinboulder"][name="diffMinboulder"]', grade)
        page.select_option(
            'select[id="diffMaxboulder"][name="diffMaxboulder"]', grade)
            ...
            try:
                ...
                for state in states:
                    ...
                    for grade in grades:
                        ...
                        filtered_url = set_route_finder_filters(
                            page, grade, state)
                        if not filtered_url:
                            print(
                                f"Failed to set filters for {state}, grade {grade}")
                            continue
                        ...
                        for page_num in range(1, total_pages + 1):
                            try:
                                print(
                                    f"Processing page {page_num}/{total_pages} for {state} grade {grade}")

                                # Navigate to specific page if not on first
                                # page
                                if page_num > 1:
                                    page.goto(
                                        f"{filtered_url}&page={page_num}")
                                    page.wait_for_selector(".table-responsive")

                                route_ids_to_check = {}

                                # Get route data from current page
                                route_rows = page.query_selector_all(
                                    ".table-responsive .route-table.hidden-xs-down .route-row")

                                for row in route_rows:
                                    route_link = row.query_selector(
                                        "a").get_attribute("href")
                                    route_name = row.query_selector(
                                        "strong").text_content().strip()
                                    route_id = route_link.split(
                                        '/route/')[1].split('/')[0]
                                    route_ids_to_check[route_id] = (
                                        route_name, route_link)

                                # Check if route already exists before
                                # processing
                                ...
                                for route_id, (route_name, route_link) in route_ids_to_check.items(
                                    ...
                                    if route_html_content == "BROWSER_CLOSED":
                                        print(
                                            "Browser closed, recreating session...")
                                        if context:
                                            try:
                                                context.close()
                                    ...
                                    queries.insert_routes_batch(
                                        cursor, [current_route_data])
                                    queries.insert_comments_batch(
                                        cursor, current_route_comments_data)
                                    conn.commit()
                            except Exception as e:
                                print(f"Error processing page: {str(e)}")
                                conn.rollback()
                                if "context or browser has been closed" in str(
                                        e):
                                    if context:
                                        context.close()
                                    if browser:
        ...
        with sync_playwright() as playwright:
            try:
                ...
                for route_id in route_ids:
                    ...
                    existing_routes = queries.check_routes_exists(cursor, [
                                                                  route_id])
                    if int(route_id) in existing_routes:
                        print(
                            f"Route with ID {route_id} already exists in database.")
                        continue
