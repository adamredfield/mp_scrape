...
def create_drill_down_bars(
        conn, tag_type, related_data, user_id, year_start, year_end, route_types):
    ...
    if not st.session_state.selected_tag:
        ...
        with stylable_container(
            ...
            for i, (_, tag, count) in enumerate(
                    zip(tag_df['Type'], tag_df['Tag'], counts), 1):
                percentage = (count / max_count) * 100 if max_count > 0 else 0

                row = st.container()
                with row:
                    if st.button("", key=f"btn_{tag}",
        ...
        filtered_df = related_df[
            ...
            (related_df['id'].str.startswith(
                f"{st.session_state.selected_tag}_"))
    ...
    related_tag = st.selectbox(
        ...
        options=[
            opt for opt in [
                ...
                'feature',
                'descriptor',
                'rock_type'] if opt != st.session_state.primary_tag],
        format_func=lambda x: {
            'style': 'Climbing Style',
            'feature': 'Route Feature',
            'descriptor': 'Route Characteristic',
