...
try:
    ...
    with st.expander("Filters"):
        ...
        if date_filter_type == "Single Year":
            ...
            with col2:
                valid_end_years = [
                    y for y in available_years if y >= year_start]
    ...
    filtered_df = df
    if selected_grades:
        filtered_df = filtered_df[filtered_df['commitment_grade'].isin(
            selected_grades)]
    if route_types:
    ...
    filtered_df = filtered_df[
        (filtered_df['date'].dt.year >= year_start) & (
            filtered_df['date'].dt.year <= year_end)
    ]
